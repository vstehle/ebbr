...
.. versionadded:: 2.1.0
   The DTB Nodes and Properties must be compliant with the requirements listed
   in [DTSPEC]_ § 3 Device Node Requirements & [DTSPEC]_ § 4 Device Bindings,
   and with the requirements listed in the following table, which take
   precedence. [#DTSchNote]_
...
.. versionadded:: 2.1.0
   ...
   Every firmware image that can be updated in-band must be described in the
   ESRT.

.. versionadded:: 2.2.0
   Firmware must support the delivery of capsules via file on mass storage
   device ("on disk") as described in :UEFI:`8.5.5`. [#VarNote]_
